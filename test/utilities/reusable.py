...
def read_csv_entity():
    ...
    cr_entity_ids = [item for sublist in cr_entity_ids for item in sublist]
    test_data_entity_ids = []

    with open('/home/tanushree/Downloads/onboard-service/test/resources/test_data/Onboarding_API - AA.csv', 'r') as csv_file:
        csv_reader = csv.DictReader(csv_file)
        for row in csv_reader:
            payload = row['payload']
            payload_dict = json.loads(payload)
    ...
    if set(cr_entity_ids) == set(test_data_entity_ids):
        ...
        same_entity_id = test_data_entity_ids[0] if test_data_entity_ids else None
        print(f"Pass the same entity ID: {same_entity_id}")
